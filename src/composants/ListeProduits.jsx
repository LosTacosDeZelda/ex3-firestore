...
export default function ListeProduits(props) {
  ...
  useEffect(() => {
    async function getProduits() {
      ...
      await database.collection("ex3-produits").get().then(
  ...
  return (
    <div className="ListeProduits">
      ...
      <ul>        
        ...
        */}
        {produits.map(produit => <Produit key={produit.id} etatPanier={props.etatPanier} nom={produit.nom} prix={produit.prix} id={produit.id}/>)}
      </ul>
    </div>
  );
}